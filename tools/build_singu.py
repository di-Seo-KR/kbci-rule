...
NEW={
 ...
 "제2조(용어의 정의)":("제2조(용어의 정의) 이 규정에서 사용하는 용어의 뜻은 다음과 같다. 1. “규정”이란 계속적인 구속력을 가지는 규범을 조문 형식으로 정한 것을 말한다. 2. “지침”이란 규정의 시행이나 보충에 필요한 사항을 조문 형식으로 정한 것으로서 계속적인 구속력을 가지는 것을 말한다. 3. “외규”란 법령이나 법령에 따라 발령된 명령·규칙, 감독기관의 규정·지침 등 계속적인 구속력을 가지는 규범을 말한다. 4. “서식”이란 규정·지침에 딸려 업무에 사용하는 일정한 양식을 말한다. 5. “전부개정”이란 기존 조문의 3분의 2 이상을 개정하거나, 여러 차례 개정으로 삭제·추가된 조문이 많아 정비할 필요가 있는 경우의 규정 개정방식을 말한다.",
   "[가독성·정합성] ★‘지침’ 순환정의→적극 정의, ‘외규’ 발하여진→발령된, ★‘서식’ 정의를 독립 업무서식(보고서·전표=문서관리규정 소관)→사규에 딸린 양식으로 정정, ~라 함은→~란"),
 ...
 "제37조(관계사항)":("제37조(관계사항) 관계있는 직명, 장소(합의처·제출처 등), 시기, 수량, 금액, 다른 규정 등은 규정에 분명히 표시한다.",
...
EXTRA=[("〈신설〉","제○조(정본 및 열람) ① 「사규관리시스템」에 등재된 사규를 정본(正本)으로 하며, 그 내용이 인쇄물 등 다른 사본과 다를 때에는 정본에 따른다. ② 임직원은 사규관리시스템을 통하여 사규를 열람한다.","[시스템·신설] 정본 채택(승인). 제15조 삭제분(열람) 흡수"),
 ("〈신설〉","제○조(별표·서식·부록의 관리) 사규에 딸린 별표·별지서식·도표·부록은 그 사규의 제정·개정·폐지 시 「사규관리시스템」에서 함께 등록·개정·삭제한다.","[시스템·신설] 사규 첨부물은 사규 제·개정으로 시스템에서 일괄 관리(삭제된 제3장 서식 운용 대체)")]
