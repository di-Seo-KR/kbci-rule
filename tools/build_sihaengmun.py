...
def build(out):
    ...
    bullet("본문 조회 : 조문 목차, 개정이력, 신구대비, 전문(全文) 다운로드 등")
    ...
    # 4. 향후 사규 관리 방안
    head(4, "향후 사규 관리 방안")
    sub("가. 사규 열람 : 전 임직원은 사규 열람 시 본 시스템을 활용")
    sub("나. 사규 제·개정 : 사규 제·개정·폐지는 사규관리시스템을 통해 등록·관리(담당 : 경영전략부)")
    sub("다. 문 의 : 경영전략부 박현민 과장 / 서동인 계장")
