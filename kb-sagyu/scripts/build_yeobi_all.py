# -*- coding: utf-8 -*-
"""여비규정 전부개정안 v4 — 심사본·전조문 신구대비표·사규서식본 3종 일괄 생성.
...
def build_review():
    ...
    para("(v4 — 출장 기준·절차 보강 + 공무원 여비 규정 대조 보완)", size=11, align=AL.CENTER, color=MUT, after=2)
    ...
    para("현행 여비규정은 여비의 계산 기준만 정하고 있어 출장 업무의 절차 규범이 존재하지 않는다. 이에 따라 "
         ...
         "이에 출장 기준을 검증 가능한 요건으로 바꾸고 절차 규범을 신설하며, 「공무원 여비 규정」(2026. 7. 1. 시행) 및 "
         "한전KDN 여비규정과 대조하여 확인된 규율 공백을 보완하고, 편제·표기를 「사규관리규정」(2026. 6. 23. 전부개정) "
         "표준에 맞추어 전부개정한다.", after=6)

    para("2. 주요 내용", size=12, bold=True, after=3)
    for t in ("가. 출장 기준의 전환(제19조) ★ : 당일출장 여비의 소요시간 3구간(3시간 미만/이상, 6시간 이상)을 폐지하고, "
              "근무지 내 출장은 출장 1회당 정액으로 지급. 판정 기준이 '몇 시간 걸렸는가'(신고 외 검증 불가)에서 "
              "'어디로 갔는가'(출장지로 자동 확정)로 바뀜 (한전KDN 여비규정 제20조 벤치마킹)",
              "나. 출장 절차 신설(제15조·제16조) ★ : 출장명령(출장명령부를 [서식 제1호]로 등재)과 복명·정산(복귀 후 7일 이내, "
              "증빙 첨부 의무) 조문 신설. 근무지 내 정액 지급 시에는 증빙 면제로 행정부담 경감",
              "다. 부정 수령에 대한 조치 신설(제17조) ★ : 반환·가산징수·징계 회부·선급 제한 "
              "(「공무원 여비 규정」 제31조·한전KDN 여비규정 제15조 참고) <가산 배수는 법무 검토 요망>",
              "라. 규율 공백 보완 : 거주지 등에서 직접 여행 시의 여비(제8조), 여비의 조정(제10조), "
              "직원이 아닌 사람의 여비(제13조), 숙박비·식비의 특례(제21조) 신설",
              "마. 계산규칙 보완 : 여행일수 계산(제6조제3항), 같은 날 여비가 둘 이상일 때(제7조제3항), "
              "장기체재 기산·일시 출장기간 공제(제22조제2항)",
              "바. 출장의 구분(제18조제1항) : 근무지 내·외 출장을 지급기준 조문에서 직접 구분 — 별도의 정의 조문은 두지 않음",
              "사. 상시·반복 업무의 처리(제19조제3항) : 소속 부점장이 일상적·반복적 고유업무로 인정하면 근무지 외에도 정액 적용",
              "아. 통제 장치 보완(제14조) : 배우자 동반 출장에 대표이사 승인 및 사전 신청 요건 신설",
              "자. 법령·현실 정합 : 검사부→감사부, 본적지→등록기준지(호주제 폐지), 부양가족 형제자매 20세→19세(민법), "
              "항공기 이용 요건 정비(KTX 보편화 반영)",
              "차. 위임 착지 정리(제39조), 원격지 근무 교통비 규정화(제38조), 재검토기한 신설(제40조), "
              "총칙 구성 정비, 전 조문 표현 정비 및 부칙 정리(종전 10개 실효)",
              "※ 법인카드 결제 원칙과 항공마일리지 규율은 개인별 법인카드 미보유·국외출장 빈도를 고려하여 이번 개정에서 제외"):
    ...
    para("아래는 문안 확정 전에 협의·확인이 필요한 사항이다. 특히 제19조의 정액 금액은 예산 협의 후 별표에 반영한다.",
         size=9, color=MUT, after=4)
    for i, n in enumerate(D.REVIEW_NOTES, 1):
        para(f"{i}. {n}", size=9.5, indent=2, after=3)

    out = os.path.join(BASE, "drafts/여비규정_전부개정안_v4.docx")
...
def build_daebi():
    ...
    out = os.path.join(BASE, "drafts/여비규정_신구조문대비표_v4.docx")
...
def build_template():
    ...
    OUT = os.path.join(BASE, "drafts/여비규정_전부개정안_v4_사규서식.docx")
    shutil.copy(SRC, OUT)
