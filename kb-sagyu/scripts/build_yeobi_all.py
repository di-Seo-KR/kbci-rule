...
def build_review():
    ...
    para("현행 여비규정은 여비의 계산 기준만 정하고 있어 출장 업무의 절차 규범이 존재하지 않는다. 이에 따라 "
         ...
         "또한 ③ '출장'·'근무지'·'시내' 등 핵심 용어가 정의되어 있지 않고, ④ 존재하지 않는 조직(검사부), 폐지된 개념(본적지), "
    ...
    for t in ("가. 출장 기준의 전환(제16조) ★ : 당일출장 여비의 소요시간 3구간(3시간 미만/이상, 6시간 이상)을 폐지하고, "
              "근무지 내 출장은 출장 1회당 정액으로 지급. 판정 기준이 '몇 시간 걸렸는가'(신고 외 검증 불가)에서 "
              "'어디로 갔는가'(출장지로 자동 확정)로 바뀜 (한전KDN 여비규정 제20조 벤치마킹)",
              "나. 출장 절차 신설(제13조·제14조) ★ : 출장명령(출장명령부를 [서식 제1호]로 등재)과 복명·정산(복귀 후 7일 이내, "
              "증빙 첨부 의무) 조문 신설. 근무지 내 정액 지급 시에는 증빙 면제로 행정부담 경감",
              "다. 정의 신설(제2조) : 출장, 근무지, 근무지 내·외 출장, 출장기간을 정의하여 적용 범위를 명확화",
              "라. 상시·반복 업무의 처리(제16조제3항) : 소속 부점장이 일상적·반복적 고유업무로 인정하면 근무지 외에도 정액 적용 "
              "— 채권추심·조사 등 상시 외근에 정식 출장여비가 지급되는 것을 방지",
              "마. 통제 장치 보완(제12조) : 배우자 동반 출장에 대표이사 승인 및 사전 신청 요건 신설",
              "바. 법령·현실 정합 : 검사부→감사부, 본적지→등록기준지(호주제 폐지), 부양가족 형제자매 20세→19세(민법), "
              "항공기 이용 요건 정비(KTX 보편화 반영)",
              "사. 위임 착지 정리(제35조) : '별도로 정하는 바에 따른다'(착지 없음) → 대표이사 위임으로 명시",
              "아. 원격지 근무 교통비 규정화(제34조) : 시행문으로만 운영해 온 지급 기준을 조문화 <위치·중복 여부 확인 요망>",
              "자. 재검토기한 신설(제36조), 총칙 표준 편제 적용, 전 조문 표현 정비 및 부칙 정리(종전 10개 실효)"):
    ...
    para("아래는 문안 확정 전에 협의·확인이 필요한 사항이다. 특히 제16조의 정액 금액은 예산 협의 후 별표에 반영한다.",
